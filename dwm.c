...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	w = showsystray && systray ? systrayspacing + horizpadbar : horizpadbar;
	i = -1;
	while (text[++i]) {
		if (text[i] == '^') {
			if (!isCode) {
...
runautostart(void)
{
	...
	if (xdgconfighome != NULL && *xdgconfighome != '\0') {
		...
		/* space for path segments, separators and nul */
		pathpfx = ecalloc(1, strlen(home) + strlen(config)
		                     + strlen(dwmdir) + 3);

		if (sprintf(pathpfx, "%s/%s/%s", home, config, dwmdir) < 0) {
			free(pathpfx);
			return;
		}
	}
...
updatesystray(void)
{
	...
	w = w ? w + systrayspacing + horizpadbar : 1;
	x -= w;
